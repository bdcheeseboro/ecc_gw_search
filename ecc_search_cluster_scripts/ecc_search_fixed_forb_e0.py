...
#This script runs a targeted detection eccentric search on a portion of the dataset.
# 
# Updated: 08/04/2021
...
def get_ew_groups(pta):
    ...
    if 'q' in pta.params:
        gpars = ['log10_Mc', 'q', 'gamma0', 'l0', 'psi'] #global params
        ...
        for pd, pp, gp in zip(pdist_params, pphase_params, gammap_params):
            ...
            groups.extend([[params.index(pd), params.index(pp), params.index(gp), params.index('log10_Mc'), params.index('q')]])
...
parser.add_argument('--psrs',
                    action='store_true', required=False, default=False,
                    help='If true it uses 11y psrs. If false it uses 12.5 yr psrs.')
parser.add_argument('--crn_type',
                    action='store', required=False, default='vary',
                    help='choose vary gwb parameters options =  [vary, fixed]')
parser.add_argument('--bayesephem',
                    action='store_true', required=False, default=False,
                    help='turn on bayesephem')
...
#select problematic pulsars from the list of pulsars
psrs_fix = []
for psr in psrs:
    if psr.name in PsrList_nfix:
        psrs_fix.append(psr)

#select noise params for only the problematic pulsars
noise_dict = {}
for psr in PsrList_nfix:
...
e0 = parameter.Constant(0.001)('e0') #eccentricity
...
ew_params = ['log10_Mc', 'q', 'l0', 'gamma0', 'psi']
for ew in ew_params:
    sampler.addProposalToCycle(jp.draw_from_par_prior(ew),5)


N = int(1.5e6)
#print(pta.get_lnlikelihood(xecc))
#print(pta.get_lnprior(xecc))
