...
if os.path.exists(filename):
    ...
    for p, t in zip(parfiles, timfiles):
        ...
        psrs.append(Pulsar(p, t, ephem='{0}'.format('DE436')))
...
#Set Tspan
tmin = np.array([p.toas.min() for p in psrs])
tmax = np.array([p.toas.max() for p in psrs])
